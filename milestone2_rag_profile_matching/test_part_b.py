...
JOB_DATASET = Path(__file__).resolve().parent / "job_descriptions.json"


def test_job_dataset():
    jobs = json.loads(JOB_DATASET.read_text(encoding="utf-8"))
    assert len(jobs) >= 5
...
def test_candidate_matching():
    ...
    job_data = json.loads(JOB_DATASET.read_text(encoding="utf-8"))[0]
